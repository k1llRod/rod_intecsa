...
class HelpdeskSLAStatus(models.Model):
    ...
    @api.depends('sh_ticket_id.create_date', 'sh_sla_id',
                 'sh_ticket_id.stage_id')
    def _compute_sh_deadline(self):
        for rec in self:

            if rec.sh_sla_id.sh_sla_target_type == 'assign_to' and rec.sh_sla_stage_id == rec.sh_ticket_id.stage_id:
                deadline = fields.Datetime.now()
            elif rec.sh_sla_id.sh_sla_target_type == 'assign_to' and rec.sh_sla_stage_id:
                rec.deadline = False
                continue
            else:
                deadline = rec.sh_ticket_id.create_date
            working_calendar = rec.sh_ticket_id.team_id.sh_resource_calendar_id
            if not working_calendar:
                # Normally, having a working_calendar is mandatory
                rec.sh_deadline = deadline
                continue

            time_days = rec.sh_sla_id.sh_days
            if time_days and (
                    rec.sh_sla_id.sh_sla_target_type == 'reaching_stage'
                    or rec.sh_sla_id.sh_sla_target_type == 'assign_to'
                    and not rec.sh_sla_id.sh_stage_id):
                ...
                create_dt = rec.sh_ticket_id.create_date
            ...
            elif time_days and rec.sh_sla_id.sh_sla_target_type == 'assign_to' and rec.sh_sla_stage_id == rec.sh_ticket_id.stage_id:
                deadline = working_calendar.plan_days(time_days + 1,
                                                      deadline,
                                                      compute_leaves=True)
            ...
            sla_hours = rec.sh_sla_id.sh_hours + (rec.sh_sla_id.sh_minutes /
                                                  60)

            if rec.sh_sla_id.sh_sla_target_type == 'reaching_stage':
            ...
            rec.sh_deadline = working_calendar.plan_hours(sla_hours,
                                                          deadline,
                                                          compute_leaves=True)
...
class HelpdeskTicket(models.Model):
    ...
    sh_merge_ticket_ids = fields.Many2many('helpdesk.ticket',relation='model_merge_helpdesk_ticket',column1="helpdesk", culomn2="ticket", string='Merge Tickets')
