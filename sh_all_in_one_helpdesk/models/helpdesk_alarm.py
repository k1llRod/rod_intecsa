...
class TicketAlarm(models.Model):
    ...
    sh_reminder_unit = fields.Selection([('Hour(s)', 'Hour(s)'), (
        'Minute(s)', 'Minute(s)'), ('Second(s)', 'Second(s)'), ('days(s)', 'Days(s)'), ('weeks(s)', 'Weeks(s)'),
                                         ('months(s)', 'Months(s)')], string="Reminder Unit", default='Hour(s)',
                                        required=True)
    ...
    def _check_sh_reminder_unit(self):
        ...
        elif self.filtered(lambda c: c.sh_reminder_unit == 'days(s)' and c.sh_remind_before < 3):
            raise ValidationError(_("Reminder Before can't set less than 5 Days."))
        elif self.filtered(lambda c: c.sh_reminder_unit == 'weeks(s)' and c.sh_remind_before < 1):
            raise ValidationError(_("Reminder Before can't set less than 1 weeks."))
        elif self.filtered(lambda c: c.sh_reminder_unit == 'months(s)' and c.sh_remind_before < 1):
            raise ValidationError(_("Reminder Before can't set less than 1 months."))
