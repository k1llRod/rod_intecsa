...
class DownloadReport(http.Controller):
    ...
    def download_ticket(self, ticket_id, report_type=None, access_token=None, message=False, download=False, **kw):
        ...
        report_type = 'pdf'
        download = True
        return self._show_report(model=ticket_sudo, report_type=report_type, report_ref='sh_all_in_one_helpdesk.action_report_helpdesk_ticket', download=download)
...
class HelpdeskTicketFeedbackController(http.Controller):
    ...
    def helpdesk_ticket_feedback(self, ticket_id, **kw):
        return http.request.render('sh_all_in_one_helpdesk.helpdesk_ticket_feedback_page', {'ticket': ticket_id})
    ...
    def get_ticket_counter_data(self, **kw):
        ...
        for stage in company_id.dashboard_filter:
            ...
            if int(kw.get('team')) != 0:
                ...
                if request.env.user.has_group('sh_all_in_one_helpdesk.helpdesk_group_team_leader') and request.env.user.has_group('sh_all_in_one_helpdesk.helpdesk_group_user') and not request.env.user.has_group('sh_all_in_one_helpdesk.helpdesk_group_manager'):
                    team_ids = request.env['helpdesk.team'].sudo().search(
                        ['|', ('team_head', '=', request.env.user.id), ('team_members', 'in', [request.env.user.id])])
                    doman.append(('team_id', 'in', team_ids.ids))
                elif not request.env.user.has_group('sh_all_in_one_helpdesk.helpdesk_group_team_leader') and request.env.user.has_group('sh_all_in_one_helpdesk.helpdesk_group_user') and not request.env.user.has_group('sh_all_in_one_helpdesk.helpdesk_group_manager'):
                    team_ids = request.env['helpdesk.team'].sudo().search(
        ...
        return request.env['ir.ui.view'].with_context()._render_template('sh_all_in_one_helpdesk.ticket_dashboard_count', {
    ...
    def get_ticket_table_data(self, **kw):
        ...
        for stage in company_id.dashboard_tables:
            ...
            if search_stage:
                ticket_data_dic.update({search_stage.name: ticket_list})
                ticket_data_list.append(search_stage.name)
        return request.env['ir.ui.view'].with_context()._render_template('sh_all_in_one_helpdesk.ticket_dashboard_tbl', {
            'ticket_data_dic': ticket_data_dic,
            'ticket_data_list': ticket_data_list,
        })
