...
class VrPrepareData(models.Model):
    ...
    def prepare_data_for_invoice(self, obj_inv, use_native_discount=False):
        data = {
        ...
        for line in obj_inv.invoice_line_ids.filtered(lambda l: l.price_unit > 0 and l.display_type != 'line_note' and l.display_type != 'line_section'):
            ...
            # NOMBRE PRODUCTO
            nombre_producto = self.env['vr.operations'].get_nombre_producto(line=line.product_id)
            detalle.append({
                'codigo_producto': str(line.product_id.vr_codigo_producto),
                'codigo_producto_cliente': str(nombre_producto),
    ...
    def prepare_data_for_cancellation(self, obj_inv, pos=False):
        if obj_inv.vr_codigo_motivo:
            data = {
                ...
                'numero_factura': obj_inv.vr_numero_factura,
    ...
    def prepare_data_for_get_pdf(self, obj_inv, type, pos=False):
        data = {
    ...
    def prepare_data_for_get_xml(self, obj_inv, pos=False):
        data = {
            'codigo_sucursal': str(obj_inv.vr_warehouse_id.vr_codigo_sucursal) if not pos else str(
                obj_inv.config_id.vr_codigo_sucursal),
            'codigo_pos': '0' if not pos else str(obj_inv.config_id.vr_codigo_pos),
    ...
    def prepare_data_for_get_report_excel(self, params):
        data = {
            'cuis_codes': params.cuis_codes,
            'year': params.year,
            'month': params.month
        }
        return data