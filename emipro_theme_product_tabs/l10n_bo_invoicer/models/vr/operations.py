...
class VrOperations(models.Model):
    ...
    def prepare_url_and_headers(self, service='invoicer'):
        try:
            ...
            # OBTENER XML
                ...
                headers = {}
            # OBTENER REPORT EXCEL
            elif service == 'get_excel':
                url += '/invoicer/get_invoices_excel/' + api_key
                headers = {}
            elif service == 'get_invoice_number':
                url += '/invoicer/get_invoice_number/' + api_key
                headers = {}
            # OBTENER LISTADO CUIS
            elif service == 'get_cuis_list':
                url += '/help/get_cuis_list/' + api_key
                headers = {}
            # COMPROBAR FACTURAS PENDIENTES
            elif service == 'check_pending_invoices':
                url += '/invoicer/check_pending_invoices/' + api_key
                headers = {}
            return url, headers
    ...
    def send_invoice_backend(self):
        try:
            ...
            if response['estado'] == 200:
                ...
                obj_inv.vr_estado = 'send_and_confirm' if response['descripcion'] == 'VALIDADA' else 'pending'
                # DATOS EXTRAS
                if 'datos_factura' in response:
                    obj_inv.vr_cuf = response['datos_factura']['cuf']
                    obj_inv.vr_fecha_emision = response['datos_factura']['fecha_emision']
                    obj_inv.vr_leyenda = response['datos_factura']['leyenda']
                    obj_inv.vr_leyenda_rep_graf = response['datos_factura']['leyenda_rep_graf']
                    if 'qr' in response['datos_factura']:
                        qr_txt = str(response['datos_factura']['qr']).encode('utf-8')
                        attachment = self.env['ir.attachment'].create({
                            'name': _("QR_%s_%s.png", str(numero_factura), str(obj_inv.vr_warehouse_id.vr_codigo_sucursal)),
                            'res_id': obj_inv.id,
                            'res_model': 'account.move',
                            'res_field': 'vr_attachment_qr',
                            'mimetype': 'image/png',
                            'type': 'binary',
                            'datas': qr_txt,
                        })
                        obj_inv.vr_attachment_qr = attachment.id
    ...
    def save_in_siat_operation_data(self, response):
        try:
            ...
            siat_operationd_data = self.env['siat.operation.data'].create({
                ...
                'vr_razon_social_emisor': response['razon_social_emisor'],
    ...
    # OBTENER REPORTE DE VENTAS EN FORMATO EXCEL
    def get_sales_report_excel(self, params):
        try:
            url, headers = self.prepare_url_and_headers(service='get_excel')
            data = self.env['vr.prepare.data'].prepare_data_for_get_report_excel(params)
            response = requests.post(url, json=data, headers=headers)
            if response and response.status_code == 200:
                # GUARDAR ARCHIVO EN ATTACHMENT
                attachment = self.env['ir.attachment'].create({
                    'name': 'Reporte Excel_' + datetime.now().strftime('%Y%m%d%H%M%S') + '.xlsx',
                    'datas': base64.b64encode(response.content),
                    # 'res_model': 'account.move' if not params.pos == "0" else 'pos.order',
                    # 'res_id': obj_inv.id,
                ...
                e = response.json()
                raise ValidationError(_(e['description']))
        except Exception as error:
            raise ValidationError(_(str(error)))

    # OBTENER EL NOMBRE DEL PRODUCTO DEPENDIENDO SU TIPO
    def get_nombre_producto(self, line):
        if line.vr_tipo_nombre_producto == 'internal':
            if line.default_code:
                nombre_producto = line.default_code
            else:
                raise ValidationError(
                    _('La "Referencia Interna" del producto (%s) esta en blanco.', line.display_name))
        elif line.vr_tipo_nombre_producto == 'barcode':
            if line.barcode:
                nombre_producto = line.barcode
            else:
                raise ValidationError(
                    _('El "Barcode" del producto (%s) esta en blanco.', line.display_name))
        else:
            raise ValidationError(
                _('El producto (%s) no cuenta con "Tipo de nombre de Producto".', line.display_name))
        return nombre_producto

    # OBTENER LISTA DE CUIS
    def get_cuis_list(self):
        try:
            url, headers = self.prepare_url_and_headers(service='get_cuis_list')
            ...
            if isinstance(error, ValidationError):
                ...
                raise ValidationError(
                    _('No existe conexión con el servidor Versatil para la obtención del listado de CUIS.\nIntente nuevamente o comuníquese con soporte técnico.'))

    # VERIFICAR FACTURAS PENDIENTES
    def check_pending_invoices(self):
        self.send_pending_invoices_to_middleware()
        self.send_pending_invoices_to_middleware(pos=True)

    # ENVIAR FACTURAS PENDIENTES PARA COMPROBAR ESTADO EN SERVIDOR VERSATIL
    def send_pending_invoices_to_middleware(self, pos=False):
        try:
            url, headers = self.env['vr.operations'].prepare_url_and_headers(service='check_pending_invoices')
            json_pending_invoices=[]
            invoices = self.env['pos.order'].search([('vr_estado', '=', 'pending'),('vr_cuf', '!=', False)]) if pos else self.env['account.move'].search([('vr_estado', '=', 'pending')])
            if invoices:
                for inv in invoices:
                    json_pending_invoices.append({
                        'order_id': inv.id,
                        'cuf': inv.vr_cuf or '',
                    })
                data = {
                    'json_pending_invoices': json_pending_invoices,
                }
                response = requests.post(url, json=data, headers=headers)
                response = json.loads(response.text)
                if response['estado'] == 200:
                    for inv in response['facturas_procesadas']:
                        if pos:
                            obj_po = self.env['pos.order'].browse(inv['order_id'])
                            if inv['state'] == 'valid':
                                obj_po.vr_estado = 'send_and_confirm'
                            else:
                                obj_po.vr_estado = inv['state']
                        else:
                            obj_po = self.env['account.move'].browse(inv['order_id'])
                            if inv['state'] == 'valid':
                                obj_po.vr_estado = 'send_and_confirm'
                            else:
                                obj_po.vr_estado = inv['state']
                else:
                    raise ValidationError(_(response['mensajes']))
        except Exception as error:
            raise ValidationError(_(str(error)))
