...
class ThemePrimeMainClass(http.Controller):
    ...
    def get_brands(self, fields=['id'], options={}):
        # read_group we only want brands that are associated with product brand with count 0 should be not displayed in snippet
        # right now product_count field in brand is compute so we can't add it in domain
        brands = request.env['product.template'].read_group(domain=request.website.website_domain(), fields=['dr_brand_id'], groupby=['dr_brand_id'])
        brand_ids = []
        for brand in brands:
            if brand.get('dr_brand_id_count', 0) and brand.get('dr_brand_id'):
                brand_ids.append(brand.get('dr_brand_id')[0])
        return request.env['dr.product.brand'].search_read([('id', 'in', brand_ids)], limit=options.get('limit', 12), fields=fields)
