    ...
    def submit_for_approval(self):
        ...
        for order in self:
            ...
            if order.date_order:
                vels.update(
                    {
                        "commitment_date": fields.Date.to_string(
                            datetime.now()
                            + timedelta(order_type.quotation_commitment_days)
                        )
                    }
                )
            order.update(vels)
            # Order line values
            line_vels = {}
            line_vels.update({"route_id": order_type.route_id.id})
            order.order_line.update(line_vels)
