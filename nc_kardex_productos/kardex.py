...
class kardex_productos_inventario(models.TransientModel):
    ...
    """
        ...
        worksheet.write(4, 13, _('resource_purchase_id'), column_heading_style)
        worksheet.write(4, 14, _('date_invoice'), column_heading_style)
        worksheet.write(4, 15, _('partner_id'), column_heading_style)
        worksheet.write(4, 16, _('Note'), column_heading_style)
        worksheet.write(4, 17, _('Inventory'), column_heading_style)
        ...
        for todfact in todo_reporte:
            ...
            worksheet.write(tf + ini, 12, todfact.picking_id.name)
            worksheet.write(tf + ini, 13, todfact.resource_purchase_id.name)
            worksheet.write(tf + ini, 14, todfact.date_invoice, date_format)
            worksheet.write(tf + ini, 15, todfact.partner_id.name)
            worksheet.write(tf + ini, 16, todfact.note, number_format)
            worksheet.write(tf + ini, 17, todfact.inventario.name)
...
""" + query_total + """
        ...
        if self.aplica == "todas":
            query_saldo_anterior_param = (producto,compan, producto, compan,date_from)
    ...
    """
        ...
        if self.aplica == "todas":
            query_movimiento_param = (producto,compan, producto, compan, date_from, date_to)
        ...
        for mov in movim:
            for kardex in self:
                ...
                resource_purchase_id = mov['resource_purchase_id']
                date_invoice = mov['date_invoice']
                ...
                line = ({'date': fecha, 'concepto': concepto, 'u_entrada': u_entrada,
                         ...
                         'origin': origin, 'picking_id': picking_id, 'partner_id': partner_id, 'date_invoice': date_invoice, 'resource_purchase_id': resource_purchase_id, 'note': note, 'inventario': inventario,
                         })
                lines = [(0, 0, line)]
                kardex.write({'obj_kardex': lines})
        self._saldo_final()
    ...
    """ + query_total + """
        ...
        if self.aplica == "todas":
            query_saldo_final_param = (producto,compan, producto, compan, date_to)
...
v_saldo,state,origin,reference,resource_purchase_id,date_invoice,partner_id,note,usage,complete_name,ubicacion,inventory_id ,picking_id  
from (
-------------3)Comienza el segundo select
  ...
  ,origin,reference,resource_purchase_id,date_invoice,partner_id,note,usage,
...
--------------- EMPIEZA LA UNION 
    ...
    costo_unit
		, v_entrada,v_salida,v_saldo,state,origin,reference,resource_purchase_id,date_invoice,partner_id,note,
		usage,complete_name,ubicacion,create_uid,inventory_id,picking_id   
		from 
		(
			select sm.id,sm.create_date  AT TIME ZONE 'UTC' as date_expected,sm.product_id,sm.name,sm.company_id,sm.product_qty as u_entrada,(sm.product_qty * 0)u_salida,
			(am.amount_total_signed/sm.product_qty ) as costo_unit
			,(am.amount_total_signed) as v_entrada,(sm.product_qty  * 0)v_salida,(sm.product_qty  *0)v_saldo,sm.state,sm.origin,sm.reference,sm.resource_purchase_id,sm.date_invoice,sm.partner_id,sm.note,
		...
		from
		   (
				select sm.id,sm.create_date AT TIME ZONE 'UTC' as date_expected,sm.product_id,sm.name,sm.company_id,(sm.product_qty * 0) as u_entrada,
                                 ...
                                 ,(sm.product_qty *0)as v_entrada,(am.amount_total_signed )v_salida,(sm.product_qty  *0)v_saldo,
                                  sm.state,sm.origin,sm.reference,sm.resource_purchase_id,sm.date_invoice,sm.partner_id,sm.note
    ...
    def _buscar_factura(self):
        ...
        """
                ...
                movim = self.env.cr.dictfetchall()
                for mov in movim:
                    #facturas=self.env['account_invoice'].search([('origin','=',fact.origin)])
...
class kardex_productos_inventario_detalle(models.TransientModel):
    ...
    resource_purchase_id = fields.Many2one('purchase.order', string='Documento')
    date_invoice = fields.Date(string='Fecha de Factura')
    partner_id = fields.Many2one('res.partner', string='Proveedor')
    note = fields.Char(string='Note')
    #inv = fields.Float('Factura', related='account_invoice.n_factura',store=True)
